...
class UpsampleDWSConvInRelu(nn.Module):
    def __init__(self, channels_in, channels_out, kernel_size, upsample, stride=1, activation=nn.ReLU):
        super(UpsampleDWSConvInRelu, self).__init__()
        self.n_params = channels_out * 2
        self.upsample = upsample
        self.channels = channels_out
        ...
        reflection_padding = int(np.floor(kernel_size / 2))
        self.reflection_pad = nn.ReflectionPad2d(reflection_padding)
        self.conv = nn.Sequential(nn.Conv2d(channels_in, channels_in, kernel_size, stride, groups= channels_in),
...
class DWSResidualBlock(nn.Module):
    ...
    # see https://cs.stanford.edu/people/jcjohns/papers/fast-style/fast-style-supp.pdf
    def __init__(self,channels):
        super(DWSResidualBlock,self).__init__()
        self.n_params = channels * 4
        self.channels = channels

        self.reflection_pad = nn.ReflectionPad2d(1)
        self.conv1 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, groups= channels),
        ...
        self.fc_gamma2 = nn.Linear(100,channels)
        self.relu = nn.ReLU(inplace=False)
        self.conv2 = nn.Sequential(nn.Conv2d(channels, channels, 3, 1, groups= channels),
                                  nn.Conv2d(channels, channels, 1)) 
...
class DistilledGhiasi(nn.Module):
    def __init__(self):
        super(Ghiasi,self).__init__()
        self.layers = nn.ModuleList([
            ConvInRelu(3,32,9,stride=1),
            ConvInRelu(32,64,3,stride=2),
            ConvInRelu(64,128,3,stride=2),
            DWSResidualBlock(128),
            DWSResidualBlock(128),
            DWSResidualBlock(128),
            DWSResidualBlock(128),
            DWSResidualBlock(128),
            UpsampleDWSConvInRelu(128,64,3,upsample=2),
            UpsampleDWSConvInRelu(64,32,3,upsample=2),
            UpsampleDWSConvInRelu(32,3,9,upsample=None,activation=None)
        ])
